...
class QuantSplineLinear(nn.Linear):
...
class QuantRadialBasisFunction(nn.Module):
    def __init__(
        self,
        grid_min: float = -2.,
        grid_max: float = 2.,
...
class QuantFastKANLayer(nn.Module):
    def __init__(
        ...
        self.rbf = QuantRadialBasisFunction(grid_min, grid_max, num_grids)
        self.spline_linear = QuantSplineLinear(input_dim * num_grids, output_dim, spline_weight_init_scale)
        self.use_base_update = use_base_update
        if use_base_update:
            self.base_activation = base_activation
            self.base_linear = nn.Linear(input_dim, output_dim, dtype=torch.float32)
    ...
    def forward(self, x, use_layernorm=True):
        ...
        if self.use_base_update:
            act = self.base_activation(x)
            base = self.base_linear(act)
            ret = ret + base
        
        return self.dequant(ret)
    ...
    def plot_curve(
        ...
        '''this function returns the learned curves in a QuantFastKANLayer.
...
class QuantFastKAN(nn.Module):
    ...
    def fit(self, dataset, opt="LBFGS", steps=100, log=1, lamb=0.1,
        ...
        def closure():
            optimizer.zero_grad()
            pred = self.forward(dataset['train_input'][train_idx].to(device))
